...
// ── Live user tracking (owner-only, never exposed publicly) ──────────────────
const liveUsers = new Map(); // key → { lastSeen, path }
const LIVE_TIMEOUT = 5 * 60 * 1000; // 5 min inactivity = offline

app.use((req, res, next) => {
  const key = req.headers['x-session-token'] || getIP(req);
  liveUsers.set(key, { lastSeen: Date.now(), path: req.path });
  const cutoff = Date.now() - LIVE_TIMEOUT;
  for (const [k, v] of liveUsers.entries()) {
    if (v.lastSeen < cutoff) liveUsers.delete(k);
  }
  next();
});
...
app.get("/stats", async (req, res) => {
  ...
  res.json({
    ...
    // Live data — only you can see this (password protected)
    live_users_now:         liveUsers.size,
    actively_transcribing:  activeTranscriptions.size,
    live_user_details:      [...liveUsers.entries()].map(([k,v]) => ({
      user: k.length === 32 ? k.slice(0,8)+'…' : k,
      seen: new Date(v.lastSeen).toISOString(),
      at:   v.path
    })),
...
app.post("/create-payment", async (req, res) => {
  ...
  // Pricing: GHS 5 = 5, GHS 12 = 10, GHS 20 = 15 transcriptions. All valid 30 days.
  const packages = {
...
app.post("/transcribe", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (useToken) activeTranscriptions.add(token);
